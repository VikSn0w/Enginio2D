...
enum class Cooling     { Liquid, Air, OilCooled, Count };
enum class FuelSystem  { Carburettor, PortInjection, DirectInjection, Count };
...
const char* const* coolingNames();
const char* const* fuelSystemNames();
...
struct EngineDesign {
    ...
    // How the fuel gets in. A carburettor has no idea what lambda it is
    // delivering: it meters on airflow through a venturi and richens as that
    // rises, and nothing corrects it. Port injection holds what it is asked
    // for. Direct injection holds it too, and evaporates the whole charge
    // inside the cylinder rather than in the port, which cools it far more and
    // is most of why direct injection tolerates compression a port engine
    // cannot.
    int    fuelSystem    = static_cast<int>(FuelSystem::PortInjection);
    ...
    // ---- Cooling, oil and friction -----------------------------------------
    // What the chamber walls sit at. An air-cooled engine runs its walls a
    // great deal hotter than a water-cooled one, which costs it charge density
    // and compression ratio and is most of why it makes less power per litre.
    int    cooling       = static_cast<int>(Cooling::Liquid);
...
struct DesignSummary {
    ...
    // How long a runner's wave takes to go to the far end and come back, and
    // what that is in crank degrees at the redline. This is the quantity the
    // simulation actually has: a pipe helps when its echo returns while the
    // valve is still open, so the comparison worth making is against the valve
    // event, not against a rule of thumb.
    double intakeEchoMs   = 0.0;
    double exhaustEchoMs  = 0.0;
    double intakeEchoDeg  = 0.0;  // crank degrees at the redline
    double exhaustEchoDeg = 0.0;
...
// Presets, read from presets/*.json at first use and falling back to a table
// compiled in when that directory is missing. Index 0 is the design the
// program starts with, which is why the files are numbered.
bool                presetsAreFiles();
int                 presetCount();
const char*         presetName(int i);
EngineDesign        preset(int i);

// Designs are JSON on disk: one flat object, one field per line, so a saved
// engine can be read, diffed, hand-edited and generated by anything else.
// saveDesign and loadDesign pick the format from the extension, and still read
// the older key-per-line files.
