...
// exhaust runner pressure, so the pulses arrive separated in time and
// interfere in the collector the way they do in a real 4-into-1. On top of the
// gas noise sit the mechanical sounds an engine actually makes: valves
// seating, the crack of each combustion event, and the broadband whirr of the
// valvetrain.
// ---------------------------------------------------------------------------
class Synth {
public:
    static constexpr std::size_t kMaxCyl = 12;

    void init(std::size_t cylinders, float sampleRate);
